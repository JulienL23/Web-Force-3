...
/*
$pdo représente un objet de la classe PDO. Il "contient" notre connection à la base de données !
La classe PDO contient plusieurs méthodes (fonctions) pour effectuer des requêtes auprès de notre BDD. Dans ce fichier nouss allons voir la méthode query(). Et exec(), prepare() et execute() seront vues dans le fichier pdo_avance.php.

Query() :
Valeurs de retour :
    ->SELECT - SHOW :
        - Succés : PDOStatement (OBJ)
        -Echec : FALSE (BOOL)
...
//$pdo -> query("dhdzhdndzjhdzffju");
//Par défaut, les erreurs SQL ne s'affiche pas. Pour les afficher on ajoute le mode d'erreur WARNING au moment de la connexion à la BDD. Ceci est une options de PDO.

// 1 : DELETE
$pdo -> query("DELETE FROM employes WHERE prenom = 'Chuck'");
...
// $resultat, notre bojet PDOStatement Inexploitable, contient une fonction fetch() qui retourne les résultats de la requête sous forme d'un array.

// La fonction (méthode) fetch() prend plusieurs arguments possibles :
    // - PDO::FETCH_ASSOC : indexation associative (les noms champs deviennet les indices dans notre array)
    // - PDO::FETCH_NUM : indexation numérique (les indices sont des chiffres de 0 à N)
    // - PDO::FETCH_OBJ : indexation sous forme d'objet (les noms des champs deviennent les propriétés de l'objet)
    // - 0 argument : indexation NUM et ASSOC par defaut, mais cela est réglable dans les options PDO.
...
// La requete nous retourne plusieurs resultats donc on fait le fetch() dans une boucle.
...
// FetchAll() reçoit les même arguments que fetch() ==> PDO::FETCH_ASSOC / PDO::_FETCH_NUM / PDO::FETCH_OBJ / 0 argument.
